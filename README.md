...
- OpenWeather 대기질 지수와 미세먼지(PM10), 초미세먼지(PM2.5) 표시
...
국내 지역명을 검색했을 때 OpenGeocoding 결과가 불분명했고, `금호동`처럼 같은 이름을 사용하는 지역을 정확하게 구분하기 어려웠습니다.
...
### 3. 추가한 지역이 새로고침 후 사라지는 문제

#### 문제 상황

사용자가 검색해 추가한 카드는 메모리의 배열에만 존재해 새로고침하면 사라졌지만, localStorage에 저장한 즐겨찾기 ID는 남았습니다.

#### 해결 방법

`weatherStore`에서 추가 지역의 `locationKey`, 이름과 좌표를 `weather-added-locations`에 저장합니다. 앱을 다시 열면 저장한 좌표로 날씨를 조회해 카드를 복원합니다. 추가 지역을 삭제할 때는 연결된 즐겨찾기도 함께 제거합니다.

### 4. 상세 페이지에서 돌아오면 항상 1페이지가 되는 문제

#### 문제 상황

상세 페이지로 이동할 때 홈 컴포넌트가 제거되고, 다시 생성되면서 `currentPage = 1`과 다른 UI 상태가 초기화되었습니다.

#### 해결 방법

`WeatherHomeView`를 `KeepAlive`로 캐시하고 `onActivated`에서 30분 TTL을 확인합니다. Store는 여러 화면에서 공유할 도메인 데이터에 사용하고, 홈에서만 쓰는 페이지·검색·필터 상태는 컴포넌트에 유지했습니다. URL Query는 공유할 검색 URL이 필요하지 않고 API 결과와 스크롤 위치를 직접 보존하지 못해 선택하지 않았습니다.
